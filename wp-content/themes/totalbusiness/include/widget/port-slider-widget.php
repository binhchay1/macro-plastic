...
add_action( 'widgets_init', 'totalbusiness_port_slider_widget' );
if( !function_exists('totalbusiness_port_slider_widget') ){
	function totalbusiness_port_slider_widget() {
	...
	class Goodlayers_Port_Slider extends WP_Widget{
		...
		function __construct() {
			parent::__construct(
				...
				esc_html__('Goodlayers Port Slider Widget','totalbusiness'), 
				array('description' => esc_html__('A widget that show lastest portfolios in slider format', 'totalbusiness')));  
		}
		...
		function widget( $args, $instance ) {
			...
			if($query->have_posts()){
				echo '<div class="totalbusiness-post-slider-widget">';
				...
				while($query->have_posts()){ $query->the_post();
					...
					if( !empty($image_id) ){
						...
						echo totalbusiness_get_image($image_id, $thumbnail_size);
						echo '<div class="totalbusiness-caption-wrapper post-slider">';
						echo '<div class="totalbusiness-caption-title">';
						echo get_the_title();
						echo '</div>'; // totalbusiness-caption-title
						echo '</div>'; // totalbusiness-caption-wrapper
						echo '</a>';
						echo '</li>';
					}
				}
				echo '</ul>';
				echo '</div>'; 
				echo '</div>'; // totalbusiness-post-slider-widget
			}
			wp_reset_postdata();
		...
		function form( $instance ) {
			...
			<p>
				...
				<?php 	
				$category_list = totalbusiness_get_term_list('portfolio_category'); 
			...
			<!-- Thumbnail Size -->
			<p>
				<label for="<?php echo $this->get_field_id('thumbnail_size'); ?>"><?php esc_html_e('Thumbnail Size :', 'totalbusiness'); ?></label>		
				<select class="widefat" name="<?php echo $this->get_field_name('thumbnail_size'); ?>" id="<?php echo $this->get_field_id('thumbnail_size'); ?>">
				<?php 	
				$thumbnails = totalbusiness_get_thumbnail_list();
				foreach($thumbnails as $th_slug => $th_name){ ?>
					<option value="<?php echo totalbusiness_escape_content($th_slug); ?>" <?php if ($thumbnail_size == $th_slug) echo ' selected '; ?>><?php echo totalbusiness_escape_content($th_name); ?></option>				
				<?php } ?>	
				</select> 
			</p>			
