...
add_action( 'widgets_init', 'totalbusiness_popular_post_widget' );
if( !function_exists('totalbusiness_popular_post_widget') ){
	function totalbusiness_popular_post_widget() {
	...
	class Goodlayers_Popular_Post extends WP_Widget{
		...
		function __construct() {
			parent::__construct(
				...
				esc_html__('Goodlayers Popular Post Widget','totalbusiness'), 
				array('description' => esc_html__('A widget that show popular posts ( by comment )', 'totalbusiness')));  
		}
		...
		function widget( $args, $instance ) {
			...
			if($query->have_posts()){
				...
				while($query->have_posts()){ $query->the_post();
					...
					echo '<i class="fa ' . totalbusiness_fa_class('icon-comment') . '"></i>';
					comments_popup_link( esc_html__('0 Comment','totalbusiness'),
						esc_html__('1 Comment','totalbusiness'),
						esc_html__('% Comments','totalbusiness'), '',
						esc_html__('Comments are Off','totalbusiness') );					
					echo '</div>'; // blog-info
