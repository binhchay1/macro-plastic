(function($){
	...
	function gdlrCreateSliderSlide(textarea){
		...
		for (var i=0; i<slide_order.length; i++){ 
			var slide = $('<div class="totalbusiness-slide-wrapper" data-id="' + slide_order[i] + '" ></div>');
			...
			var remove = $('<div class="totalbusiness-delete-slide"></div>');
			remove.click(function(){
				var remove_button = $(this);
				$('body').totalbusiness_confirm({
					success: function(){
						remove_button.parent('.totalbusiness-slide-wrapper').slideUp(function(){
							$(this).remove();
							gdlrUpdateSliderOrder(textarea);
						});					
					}
	...
	function gdlrCreateSliderOption(textarea, image_id){
		...
		$('body').append(editbox);
		editbox.fadeIn(150);
		if(overlay){ $('body').addClass('totalbusiness-disable-scroll'); }
	...
	function gdlrCloseSliderOption(editbox, textarea, image_id, overlay){
		...
		if(overlay){ $('body').removeClass('totalbusiness-disable-scroll'); }	
	}
