<?php
	 ...
	 */
if( !class_exists('totalbusiness_theme_customizer') ){
	class totalbusiness_theme_customizer{
			...
			foreach($this->admin_option as $tabs){
				foreach($tabs['options'] as $section_slug => $section){
					...
					$wp_customize->add_section($section_slug, array(
						'title' => esc_html__('Color :', 'totalbusiness') . ' ' . $section['title'], 
						'priority' => $priority, 
						'capability' => 'edit_theme_options'));
					foreach($section['options'] as $option_slug => $option){
						if($option['type'] != 'colorpicker') continue;

						$wp_customize->add_setting('totalbusiness_customizer[' . $option_slug . ']', array(
						...
						$wp_customize->add_control(new WP_Customize_Color_Control($wp_customize, $option_slug,
							array(
								'label' => esc_html__('Color :', 'totalbusiness') . ' ' . $option['title'],
								'section' => $section_slug,
								'settings' => 'totalbusiness_customizer[' . $option_slug . ']',
								'priority' => $priority,
							) 
						));
						$priority++;
		...
		function sync_theme_option(){
			$theme_option = get_option(THEME_SHORT_NAME . '_admin_option', array());
			$customizer_option = get_option('totalbusiness_customizer', array());
			...
			delete_option('totalbusiness_customizer');
			
			totalbusiness_generate_style_custom($this->admin_option);
		}
...
add_action('customize_preview_init' , 'totalbusiness_register_customizer_script');
if( !function_exists('totalbusiness_register_customizer_script')){
	function totalbusiness_register_customizer_script(){
		wp_enqueue_script('totalbusiness-customize', get_template_directory_uri() . '/framework/javascript/gdlr-customizer.js', array(), '', true);	
	}
}
