<?php
	...
	*/
	...
	$totalbusiness_thumbnail_size = array(
	...
	$totalbusiness_thumbnail_size = apply_filters('totalbusiness-thumbnail-size', $totalbusiness_thumbnail_size);
	
	// create the size from totalbusiness_thumbnail_size variable
	add_action( 'after_setup_theme', 'totalbusiness_register_thumbnail_size' );
	if( !function_exists('totalbusiness_register_thumbnail_size') ){
		function totalbusiness_register_thumbnail_size(){
			add_theme_support( 'post-thumbnails' );
		
			global $totalbusiness_thumbnail_size;		
			foreach($totalbusiness_thumbnail_size as $totalbusiness_size_slug => $totalbusiness_size){
				add_image_size($totalbusiness_size_slug, $totalbusiness_size['width'], $totalbusiness_size['height'], $totalbusiness_size['crop']);
			}
		}
	}
	
	// add the image size filter to admin option
	add_filter('image_size_names_choose', 'totalbusiness_set_custom_size_image');
	if( !function_exists('totalbusiness_set_custom_size_image') ){
		function totalbusiness_set_custom_size_image( $sizes ){	
			$additional_size = array();
			
			global $totalbusiness_thumbnail_size;
			foreach($totalbusiness_thumbnail_size as $totalbusiness_size_slug => $totalbusiness_size){
				$additional_size[$totalbusiness_size_slug] = $totalbusiness_size_slug;
			}
	...
	if( !function_exists('totalbusiness_get_thumbnail_list') ){
		function totalbusiness_get_thumbnail_list(){
			global $totalbusiness_thumbnail_size, $_wp_additional_image_sizes;
			
			$sizes = array();
			foreach( get_intermediate_image_sizes() as $size ){
				if(in_array( $size, array( 'thumbnail', 'medium', 'large' )) ){
					$sizes[$size] = $size . ' -- ' . get_option($size . '_size_w') . 'x' . get_option($size . '_size_h');
				}else if( !empty($totalbusiness_thumbnail_size[$size]) ){
					$sizes[$size] = $size . ' -- ' . $totalbusiness_thumbnail_size[$size]['width'] . 'x' . $totalbusiness_thumbnail_size[$size]['height'];
				}
				//else{
			...
			$sizes['full'] = esc_html__('full size (Original Images)', 'totalbusiness');
	...
	// video size 
	if( !function_exists('totalbusiness_get_video_size') ){
		function totalbusiness_get_video_size( $size ){
			global $_wp_additional_image_sizes, $theme_option, $totalbusiness_crop_video;
			...
			// refine video size
			if( $totalbusiness_crop_video || $video_size['height'] == 9999 ){
