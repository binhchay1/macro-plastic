...
class WHOLESALEX_Registration {
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'                => array(
					...
					'save_form_changes'     => __( 'Save Form Changes', 'wholesalex' ),
	...
	public function login_redirect( $redirect, $user ) {
		...
		if ( 'yes' === $view_price_product_list || 'yes' === $view_price_product_single ) {
			$product_redirect = isset( $_POST['redirect'] ) ? wp_unslash( $_POST['redirect'] ) : $url;
			...
			if ( ! empty( $product_redirect ) ) {
				...
				$redirect_url_view_prices = isset( $_GET['redirect'] ) ? esc_url( $_GET['redirect'] ) : '';
			...
			if('file' ==$field['type'] ) {
				...
				if ( $is_valid_file) {
					...
					if ( isset( $field['allowed_file_types'] ) && ! empty( $field['allowed_file_types'] ) ) {
						$allowed_file_types = explode( ',', $field['allowed_file_types'] );
