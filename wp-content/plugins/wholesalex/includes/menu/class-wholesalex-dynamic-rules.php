...
class WHOLESALEX_Dynamic_Rules {
	...
	public function get_products( $query = '', $user_id = '', $without_child = false ) {
		...
		if ( $query ) {
			...
			$sql .= ")";
			$sql = $wpdb->prepare( $sql, '%' . $wpdb->esc_like( $query ) . '%', '%' . $wpdb->esc_like( $query ) . '%' );
		}
			...
			if ( $modal_content ) {
				...
				<div class="wsx-single-product-offers">
					...
					<div class="wsx-sp-promotions">
						<span class="wsx-single-product-offers-view-more" id="wsx-sp-dr-view-more" data-product-id="<?php echo $product->get_id(); ?>"> <?php echo esc_html( wholesalex()->get_setting( 'promo_button_text_on_sp', __( 'Get exclusive offers', 'wholesalex' ) ) ); ?> </span>

						<div class="wsx-dr-single-product-discounts-modal" id="wsx-dr-single-product-discounts-modal-<?php echo $product->get_id(); ?>" style="display:none;">
							<div class="wsx-dr-single-product-discounts-modal-body">
								<div class="wsx-dr-modal-heading"><?php echo esc_html( 'Conditional Discount Offers', 'wholesalex' ); ?></div>
				...
				<div class="wsx-bxgy-free-promo-card">
					...
					<div class="wsx-bxgy-free-item-meta">
						...
						if ( $free_item_quantity > 1 ) {
							?>
							 <div class="wsx-bxgy-free-item-qty"><?php echo sprintf( __( 'Quantity: %s', 'wholesalex' ), $free_item_quantity ); ?> </div> <?php } ?>
						<div class="wsx-bxgy-free-item-price"> <?php echo wc_price( $product->get_price( 'edit' ) * esc_html( $free_item_quantity ) ); ?> </div>
					</div>
						...
						foreach ( WC()->cart->get_cart() as $cart_item ) {
							if ( self::is_eligible_for_rule( $cart_item['product_id'], $cart_item['variation_id'], $rule['filter'] ) ) {
								...
								if ( $min_qty > 1 && $cart_item_count >= $min_qty ) {
									...
									$price = 'incl' == $cart->get_tax_price_display_mode() ? wc_get_price_excluding_tax(
										$cart_item['data'],
										array(
											'qty'   => $free_quantity,
											'price' => $price,
	...
	public function handle_discounts( $data ) {		
		...
		add_filter(
			...
			function ( $price_html, $product ) {
				...
				if ( is_admin() || ! ( is_object( $product ) && is_a( $product, 'WC_Product' ) ) ) {
					return $price_html;
				}
