		...
		switch ( $type ) {
			...
			case 'update_status':
				...
				if ( ! empty( $action ) && ! empty( $id ) ) {
					...
					$response['status'] = true;
					$response['data']   = 'delete' === $action ? 'Successfully Deleted' : 'Successfully Updated';
			...
			case 'bulk_action':
				...
				if ( empty( $ids ) || ! is_array( $ids ) ) {
					$response['status'] = false;
					$response['data']   = 'Invalid User Ids!';
				}
				...
				if ( ! empty( $action ) && ! empty( $ids ) && is_array( $ids ) ) {
					...
					} else if(wholesalex()->start_with($action,'change_role_to_')) {
