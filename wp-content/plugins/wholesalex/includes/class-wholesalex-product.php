...
class WHOLESALEX_Product {
	...
	public function save_b2b_stock_manage_fields_for_simple_product($product_id) {
		...
		if(isset($_POST['wholesalex_b2b_stock'])) {
			...
			if(intval($current_stock) == intval($original_value)) {
				...
				\WC_Admin_Meta_Boxes::add_error( sprintf( __( 'The stock has not been updated because the value has changed since editing. Product %1$d has %2$d units in stock.', 'woocommerce' ), $product_id, $current_stock ) );
			}
		}
	...
	public function b2b_stock_manage_fields_on_product_variation($loop, $variation_data, $variation ) {
		...
		woocommerce_wp_select(
			array(
				...
				'value'         => $b2b_backorders,
				'label'         => __( 'Allow WholesaleX B2B Users backorders?', 'woocommerce' ),
				'options'       => wc_get_product_backorder_options(),
				'desc_tip'      => true,
				'description'   => __( 'If managing stock, this controls whether or not backorders are allowed. If enabled, stock quantity can go below 0.', 'woocommerce' ),
	...
	public function save_b2b_stock_manage_fields_for_product_variation( $variation_id) {
		...
		if(isset($_POST["wholesalex_b2b_variable_stock_".$variation_id])) {
			...
			if(intval($current_stock) == intval($original_value)) {
				...
				/* translators: %1s Product ID, %2s Stock Count. */
				\WC_Admin_Meta_Boxes::add_error( sprintf( __( 'The stock has not been updated because the value has changed since editing. Product %1$d has %2$d units in stock.', 'woocommerce' ), $variation_id, $current_stock ) );
			}
		}
	...
	public function b2b_variation_get_stock_quantity($quantity, $product) {
		...
		if(wholesalex()->is_active_b2b_user()) {
			...
			} else if('parent' == $product->get_manage_stock()) {
	...
	public function update_product_stock( $product_id_with_stock, $stock_quantity = null, $operation = 'set' ) {
		...
		if ( 'set' === $operation ) {
			...
			// Generate SQL.
			$sql = $wpdb->prepare(
				"UPDATE %s SET meta_value = %f WHERE post_id = %d AND meta_key=%s;",
				$wpdb->postmeta,
				$new_stock,
			...
			$current_stock = wc_stock_amount(
				$wpdb->get_var( // @codingStandardsIgnoreLine.
					$wpdb->prepare(
						"SELECT meta_value FROM %s WHERE post_id = %d AND meta_key=%s;", 
						$wpdb->postmeta,
						$product_id_with_stock,
						$stock_meta_key
					)
			...
			// Generate SQL.
			$sql = $wpdb->prepare(
				"UPDATE %s SET meta_value = meta_value %+f WHERE post_id = %d AND meta_key=%s;", // @codingStandardsIgnoreLine.
				$wpdb->postmeta,
