...
class WHOLESALEX_Shortcodes {
	...
	private function renderColumns($row,$isRolewise,$inputVariation) {
		$columns         = $row['columns'];
				...
				<?php 
					foreach ($columns as $field) {
						...
						<?php 
							...
							$this->generateFormField( $field, $isRolewise, $inputVariation );
						 ?>
						</div>
						<?php
					}
		...
		<form class="wholesalex-<?php echo esc_attr($type); ?>-form" enctype="<?php echo esc_attr($enctype);  ?>">
				...
				?>
					...
					foreach ( $fields as $row ) {						
						$this->renderColumns($row,$isRolewise,$inputVariation);
					}
					if('registration'==$type) {
						if ( $isRolewise ) {
							?>
	...
	public function generateFormField( $field, $isRolewise = false, $inputVariation = '', $is_multi_column = false ) {
		if(!isset($field['name'])) {
			return;
		}
	...
	public function add_custom_fields_on_checkout_page( $checkout ) {
		...
		if ( is_array( $custom_billing_fields ) ) {
			foreach ( $custom_billing_fields as $value ) {
				if ( isset( $value['excludeRoles'] ) && is_array( $value['excludeRoles'] ) && in_array( $__role, $value['excludeRoles'] ) ) {
	...
	public function validate_custom_checkout_fields() {
		...
		if ( is_array( $__custom_fields ) && $__custom_fields && ! empty( $__custom_fields ) ) {
			foreach ( $__custom_fields as $field ) {
				if ( 'file' === $field['type'] ) {
					continue;
				} else {
					if ( isset( $field['isRequiredInBilling'] ) && ! empty( $field['isRequiredInBilling'] ) && ! ( isset( $post_data[ $field['name'] ] ) && ! empty( $post_data[ $field['name'] ] ) ) ) {
			...
			foreach ( $__custom_fields as $field ) {
				if ( 'file' === $field['type'] ) {
					...
					if ( isset($_POST[$field['name']]) && !empty($_POST[$field['name']]) ) { // phpcs:ignore
						update_post_meta( $order_id, 'wholesalex_cf_' . $field['name'], $this->sanitize_field_data( $field, $_POST[ $field['name'] ] ) ); //phpcs:ignore
					}
				}
			}
		}
	...
	public function show_custom_fields_value( $order_id ) {
		...
		if ( is_array( $__custom_fields ) ) {
			...
			?>
            <div class="wholesalex_custom_fields">
            <?php
			foreach ( $__custom_fields as $field ) {
				$__value = get_post_meta( $order_id, 'wholesalex_cf_' . $field['name'], true );
				if ( $__value && ! empty( $__value ) ) {
                    ?>
                    <label><?php echo esc_html( $field['label'] ); ?></label> : <strong><?php echo esc_html( $__value ); ?></strong><br/>
                    <?php
				}
			}
	...
	public function show_custom_fields_on_order_page( $order ) {
		...
		if ( is_array( $__custom_fields ) && $__custom_fields && ! empty( $__custom_fields ) ) {
			...
			foreach ( $__custom_fields as $field ) {
				...
				if ( $__value && ! empty( $__value ) ) {
                    ?>
                    <p class="form-field form-field-wide"> <strong><?php echo esc_html( $field['label'] ); ?>:</strong><div><?php echo esc_html( $__value ); ?></div></p>
                    <?php
				}
			}
			?>
			...
			if ( isset( $_POST['username'], $_POST['password'] ) ) {
				...
				try {
					...
					if ( empty( $creds['user_login'] ) ) {
						$data['error_messages']['username'] = __( 'Username is Required!', 'woocommerce' );
					...
					if ( is_wp_error( $user ) ) {
						...
						$data['redirect'] = wp_validate_redirect( apply_filters( 'woocommerce_login_redirect', $redirect, $user ), wc_get_page_permalink( 'myaccount' ) );
