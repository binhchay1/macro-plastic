                        ...
                        foreach ( $recent_posts as $value ) {
                            $post_loop .= '<div class="wopb-block-item">';
                                $post_loop .= '<div class="wopb-block-content-wrap">';
                                    if ( $attr['showImage'] ) {
                                        if ( $value['image'] ) {
                                            $post_loop .= '<div class="wopb-block-image wopb-block-image-'.sanitize_html_class($attr['imgAnimation']).'"><a href="'.esc_url($value['url']).'" class="wopb-product-cat-img"><img src='.esc_url($value['image'][$image_size]).' alt='.esc_html($value['name']).'/></a></div>';
                                        } else {
                                    ...
                                    if ( $attr['titleShow'] || $attr['countShow'] || $attr['descShow'] || $attr['readMore'] ) {
                                        $post_loop .= '<div class="wopb-category-content-items wopb-category-content-'.sanitize_html_class($attr['contentVerticalPosition']).' wopb-category-content-'.sanitize_html_class($attr['contentHorizontalPosition']).'">';
                                            $post_loop .= '<div class="wopb-category-content-item">';
                                                ...
                                                if ( $attr['descShow'] ) {
                                                    $post_loop .= '<div class="wopb-product-cat-desc">'.wc_format_content( wp_kses_post( $value['desc'] ) ).'</div>';
                                                }
                                                if ( $attr['readMore'] ) {
